...
class Bat:
    #### constructor
    def __init__(self, table, width=15, height=100, x_posn=50, y_posn=50, colour="green", x_speed=10, y_speed=10):
    ...
    def detect_collision(self, ball, sides_sweet_spot=True, topnbottom_sweet_spot=False):
        ...
        if (collision):
            ...
            if sides_sweet_spot and (collision_direction == "W" or collision_direction == "E"):
                # finding out how far from the centre of the bat the collision was
                adjustment = (-(v_centre - v_centre_b)) / (self.height / 2)
                ball.y_speed = BAT_FEEL * adjustment
            if topnbottom_sweet_spot and (collision_direction == "N" or collision_direction == "S"):
                # finding out how far from the centre of the bat the collision was
                adjustment = (-(h_centre - h_centre_b)) / (self.width / 2)
                ball.x_speed = BAT_FEEL * adjustment
    ...
    def move_left(self, master):
        ...
        y1 = self.y_posn
        y2 = self.y_posn + self.height
        self.table.move_item(self.rectangle, x1, x2, y1, y2)
