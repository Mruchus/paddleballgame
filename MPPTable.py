...
# A class defining a Table which is a 2D rectangular play area
...
class Table:
    ...
    def __init__(self, window, colour="black", net_colour="green",
                 ...
                 vertical_net=False, horizontal_net=False, net_width=2):
        self.width = width
        self.height = height
        self.colour = colour


        # order a canvas to draw on
        ...
        if(vertical_net):
            self.canvas.create_line(self.width/2, 0, self.width/2,
                                    self.height, width=net_width, fill=net_colour, dash=(15, 23))
        if(horizontal_net):
            self.canvas.create_line(0, self.height/2, self.width,
                                    self.height/2, width=net_width, fill=net_colour, dash=(15,23))

        # add scoreboard
        font = ("american typewriter", 72)
